...
<style>
    ...
    .editor-toolbar { display:flex; align-items:center; justify-content:space-between; gap:14px; flex-wrap:wrap; background:#f3eeff; border:1px solid #ddd6fe; border-radius:12px; padding:12px 18px; margin-bottom:18px; }
    .editor-toolbar .txt { font-size:12.5px; color:#5b21b6; font-weight:600; }
    ...
    .q-badge { font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:.4px; padding:3px 10px; border-radius:20px; background:#ecfdf5; color:#0f766e; }
    ...
    .q-reponse { width:100%; border:1.5px solid var(--border); border-radius:9px; padding:10px 12px; font-size:13px; font-family:inherit; resize:vertical; background:#f3eeff; }
...
<div class="examen-header-card">
    ...
    <div class="examen-header-meta">
        ...
        <span><i class="fa-solid fa-scale-balanced"></i> /<span id="baremeTotalDisplay">{{ $contenu['bareme_total'] ?? 20 }}</span></span>
...
@if($examen->statut === 'genere')
    ...
    <div class="editor-toolbar">
        <span class="txt"><i class="fa-solid fa-pen-to-square"></i> Mode édition — modifiez, ajoutez, supprimez ou réorganisez les questions avant l'envoi</span>
    </div>
...
@else
    {{-- ═══════════════════ MODE LECTURE SEULE (déjà envoyé) ═══════════════════ --}}
...
function activerGlisserDeposer(containerId) {
    ...
    container.addEventListener('dragstart', e => {
        // On ne démarre le glisser que depuis la poignée, pour ne pas gêner la saisie de texte
...
async function enregistrerModifications() {
    ...
    try {
        const res = await fetch('{{ route("enseignant.examens.questions.sauvegarder", $examen) }}', {
            method: 'POST',
            headers: {
